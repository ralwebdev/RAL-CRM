    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 30, fontWeight: 800, color: BRAND.secondary, textTransform: "uppercase", letterSpacing: 0.5 }}>
            {docLabel}
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 230px", marginTop: 18, gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", marginTop: 20, gap: 18, alignItems: "start" }}>
          ...
          {balance > 0 && (
            <div style={{ marginTop: 10, padding: 10, border: `1px dashed ${BRAND.primary}`, borderRadius: 8, display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ fontSize: 10, color: "#555" }}>
                Scan to pay <b>{fmtINR(balance)}</b>
                <br />
        ...
        <div style={{ width: 280, justifySelf: "end" }}>
          <table style={{ width: "100%", fontSize: 11, borderCollapse: "collapse" }}>
            <tbody>
              <tr><td style={{ padding: "6px 0", color: "#666" }}>Subtotal</td><td style={{ padding: "6px 0", textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmtINR(invoice.subtotal)}</td></tr>
              {invoice.discount > 0 && <tr><td style={{ padding: "6px 0", color: "#666" }}>Discount</td><td style={{ padding: "6px 0", textAlign: "right", color: "#10b981", fontVariantNumeric: "tabular-nums" }}>- {fmtINR(invoice.discount)}</td></tr>}
              <tr><td style={{ padding: "6px 0", color: "#666" }}>Taxable</td><td style={{ padding: "6px 0", textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmtINR(taxable)}</td></tr>
              {invoice.cgst > 0 && <tr><td style={{ padding: "6px 0", color: "#666" }}>CGST ({invoice.gstRate / 2}%)</td><td style={{ padding: "6px 0", textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmtINR(invoice.cgst)}</td></tr>}
              {invoice.sgst > 0 && <tr><td style={{ padding: "6px 0", color: "#666" }}>SGST ({invoice.gstRate / 2}%)</td><td style={{ padding: "6px 0", textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmtINR(invoice.sgst)}</td></tr>}
              {invoice.igst > 0 && <tr><td style={{ padding: "6px 0", color: "#666" }}>IGST ({invoice.gstRate}%)</td><td style={{ padding: "6px 0", textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmtINR(invoice.igst)}</td></tr>}
              <tr style={{ borderTop: "2px solid #1A1A1A" }}>
                <td style={{ padding: "8px 0", fontWeight: 800, fontSize: 13 }}>Grand Total</td>
                <td style={{ padding: "8px 0", textAlign: "right", fontWeight: 800, fontSize: 13, color: BRAND.primary, fontVariantNumeric: "tabular-nums" }}>
                  {isCredit ? "- " : ""}{fmtINR(invoice.total)}
                </td>
              </tr>
              {invoice.amountPaid > 0 && (
                <>
                  <tr><td style={{ padding: "6px 0", color: "#666" }}>Paid</td><td style={{ padding: "6px 0", textAlign: "right", color: "#10b981", fontVariantNumeric: "tabular-nums" }}>{fmtINR(invoice.amountPaid)}</td></tr>
                  <tr style={{ background: balance > 0 ? "#FEF2F2" : "#F0FDF4" }}>
                    <td style={{ padding: "8px 8px", fontWeight: 700 }}>Balance Due</td>
                    <td style={{ padding: "8px 8px", textAlign: "right", fontWeight: 700, color: balance > 0 ? BRAND.primary : "#10b981", fontVariantNumeric: "tabular-nums" }}>
                      {fmtINR(balance)}
                    </td>
                  </tr>
                </>
      ...
      <div style={{ marginTop: 32, display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16 }}>
        ...
        <div style={{ width: 200, textAlign: "center", alignSelf: "flex-end" }}>
          <div style={{ borderBottom: "1px solid #1A1A1A", marginBottom: 6, minHeight: 56, display: "flex", alignItems: "flex-end", justifyContent: "center", paddingBottom: 10, fontFamily: "'Brush Script MT', cursive", fontSize: 18, color: BRAND.primary }}>
            Authorised Sign.
          </div>
